...
function newPage(){
	window.open("claim_form_detail.jsp","_self");
}
...
function getBuyProductDetail(docNo, itemNo){
	$.ajax({
		...
		success:function(data) {
            ...
            $("#cust_doc_date").val(showDate(data.CUST_DOC_DATE));
            $("#sup_doc_no").val(data.SUP_DOC_NO);
            $("#sup_doc_date").val(showDate(data.SUP_DOC_DATE));
            $("#sup_send_date").val(showDate(data.SUP_SEND_DATE));
...
function confirmSave(e){
    $("#save-confirm").dialog({
        resizable: false,
        modal: true,
        title: "ยืนยันการบันทึกเปลี่ยนสินค้า "+$("#mode").val(),
        height: 250,
        width: 400,
        buttons: {
            "Yes": function () {
...
function saveData(e){
	$.ajax({
		...
		data: {
			...
			cust_doc_date: $("#cust_doc_date").val(),
			sup_doc_no: $("#sup_doc_no").val(),
			sup_doc_date: $("#sup_doc_date").val(),
			sup_send_date: $("#sup_send_date").val(),
			item_no: $("#item_no").val(),
			part_no: $("#part_no").val(),
...
function getDocNo(){
	$.ajax({
		...
		success:function(data){
			$("#doc_no").val("C"+data.DOC_NO);
			$("#doc_date").val(showDate(data.DOC_DATE));
			getItemNo("C"+data.DOC_NO);
		}
	});
}
