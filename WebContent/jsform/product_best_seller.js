...
function loadDataTable(){
    $('#product_best_seller_table').dataTable().fnDestroy();
	 $('#product_best_seller_table').dataTable( {
		 "ajax": {
			 type: "POST",
			 url: "./DataTableRetriveSrvl",
			 dataSrc : "data",
			 data: {
				 tb: "PRODUCT_BEST_SELLER",
				 year_end: $("#year_end").val(),
				 month_end: $("#dwl_month_end option:selected").val(),
				 year_start: $("#year_start").val(),
				 month_start: $("#dwl_month_start option:selected").val(),
				 part_no: $("#part_no").val(),
				 product_type: $("#prd_type_id").val(),
				 product_brand: $("#prd_brand_id").val(), 
				 stock: $("#stock option:selected").val(),
				 order_status: $("#dwl_order_status option:selected").val(),
